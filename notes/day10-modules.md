...
| 5 | `week2-runtime/day10-dir-size-verify.js` | 它的判据（造临时目录树 + 断言数字） | **AI 写**（✅ 9/24 已就位，你的 `dirSize` 一到位就能跑） |
| 6 | 日志 + commit + push | —— | **你** |
...
| **3:05–3:25** | **⑥ 判据**：跑 `day10-dir-size-verify.js`（✅ 已就位） | 5 |
...
7. **CLI 入口要能被安全 `require`**（接线要求，别被它坑掉半小时）：这个文件要**同时**当模块（`require` 进来拿 `dirSize`）和脚本（直接 `node` 跑），所以入口那几行要包进 `if (require.main === module) { ... }` —— 不然判据一 `require` 你的文件，你的 CLI 就当场跑起来了（万一里面还有 `process.exit()`，判据进程直接死掉，你只会看到一行莫名其妙的报错）。**判据的 00 / 08 两条就是查这个的。**
...
> 判据会在这几条的输出里找**总字节数这个数字**（带不带单位没关系，数字要出现），所以人看的报告里也要把总大小打出来。
...
它**自己造一棵已知大小的临时目录树**（在系统临时目录里，跑完删掉，不碰仓库 —— 6 个文件 / 107 字节，含两层子目录、一个中文文件、一个空目录），然后：

| 条目 | 判什么 |
|---|---|
| 00 / 08 | 模块能被 `require`、导出 `dirSize`、**CLI 入口不会在 `require` 时自己跑起来** |
| 01–05 | `totalBytes` / `fileCount` / `largest` 的数字、顺序、路径（要相对被统计目录；`\` 和 `/` 都算对） |
| 06 / 07 | 命令行跑得出总字节数；目录不存在时**友好报错**（打出调用栈就红） |
| P | **探针（只记录不判错）**：空目录 / `--top=3` / `--json` —— 属「砍单顺序」里能顺延的，没做不算红，但要登记进日志 |

红的每一条都会**告诉你差在哪、可能是什么原因**（例如"差 4 字节 → 你大概用了 `content.length` 而不是 `stat.size`"）。

> 老规矩：跑绿之前先看 `ℹ pass` 那个数。本文件 **10 条 `test()`**，如果 pass 只有个位数、大半是 skipped，**那不是绿，是没跑起来**。

**先自己写两个用例也不是不行**（今天零提示题刚练过判据）—— 那就自己先写，再跑这份。
...
- [ ] 判据 `day10-dir-size-verify.js` 跑绿（**10 条：必过 9 + 探针 1**；探针里的 `--top` / `--json` 没做就登记欠账）
- [ ] 日志 + commit + push
