...
> **今天的第一件事不是学新东西**：Day 5 还剩约 20 分钟收尾（**只剩 3 件**：加一条测试 + 三个自测题 + 读测试文件；日志三节已在 9/16 补完并提交 `90afbd8`）。**先补完再进新内容**——按你计划的规则，欠账不清不动新章节。
...
| # | 产出物 | 验收标准 |
|---|---|---|
| 0 | **Day 5 欠账 3 件**（见第二节） | 全部打勾（约 20 分钟） |
...
| 时间 | 干什么 | 产出 |
|---|---|---|
| **08:30–08:50** | **Day 5 欠账收尾（3 件）** | 产出物 0 |
| 08:50–10:10 | 读《对象方法，"this"》+《深入理解箭头函数》+ 写 `day06-this.js` | 产出物 1 |
| 10:10–10:25 | 休息 | —— |
| 10:25–11:40 | 读《原型，继承》+《class 基本语法》+《类继承》+ 写 `day06-prototype.js` | 产出物 2 |
| 11:40–13:00 | 午休，不碰屏幕 | —— |
| **13:00–13:45** | **裸写 `myMap` / `myFilter` / `myReduce`** + 跑脚手架 | 产出物 3 |
| 13:45–14:00 | 休息 | —— |
| 14:00–14:45 | 搬进工具箱 + 写测试 | 产出物 4 |
| 14:45–15:15 | AI review + 英文 15 分钟 + 日志 + commit + push | 产出物 5 |

> 今天 08:21 就开始了（比昨天早两小时），所以核心部分 **15:15 就能收工**。早收工也是计划的一部分——§二 作息模板写着"午休不碰屏幕，让大脑固化"。

---

## 二 08:30–08:50 先收尾 Day 5（3 件，约 20 分钟）

> 日志三节已在 9/16 补完并提交，所以今天的欠账只剩下面 3 件。
...
## 五 13:00–13:45 数组三个方法的 polyfill（产出物 3）
...
## 六 14:45–15:15 AI 协作 + 收尾

**AI 协作三个动作**（§二 作息模板）：

1. **让 AI review 今天代码** —— "这是我的 `day06-this.js`，请指出我哪种 `this` 绑定理解错了，**不要给我重写**"
2. **让 AI 出 3 道题** —— "出 3 道关于 `this` 绑定和原型链的题，一次一题，由易到难，等我回答再给下一题"
3. **让 AI 指出今天最含糊的概念**
...
```powershell
cd C:Users971.zcodeworkspacedefaultjs-node-30days
git status
git add -A
git status
...
**关键检查点 10:10**：如果那时 `this` 还没写完演示，就把原型链那一章的阅读压到 30 分钟、先保住 array polyfill 的裸写时段。
